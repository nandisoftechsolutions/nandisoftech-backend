...
exports.createVideo = async (req, res) => {
  try {
    const {
      title = '', description = '', uicode = '', backendcode = '',
      youtubeLink = '', course_id = null, teacher_id = null
    } = req.body;

    const thumbnail = req.files?.thumbnail?.[0]?.filename || null;
    const videoFile = req.files?.videos?.[0]?.filename || null;
    ...
    const result = await pool.query(
      `INSERT INTO courseVideos 
       ...
       RETURNING *`,
      [title, description, uicode, backendcode, youtubeLink, thumbnail, videoFile, course_id, teacher_id]
    );
...
exports.updateVideo = async (req, res) => {
  try {
    ...
    let videoFile = req.files?.videos?.[0]?.filename;

    const existing = await pool.query('SELECT thumbnail, videos FROM courseVideos WHERE id = $1', [req.params.id]);

    if (!thumbnail) thumbnail = existing.rows[0]?.thumbnail || null;
    if (!videoFile) videoFile = existing.rows[0]?.videos || null;

    const result = await pool.query(
      `UPDATE courseVideos SET
         title = $1, description = $2, uicode = $3, backendcode = $4,
         youtubelink = $5, thumbnail = $6, videos = $7,
         course_id = $8, teacher_id = $9
       WHERE id = $10
       RETURNING *`,
      [title, description, uicode, backendcode, youtubeLink, thumbnail, videoFile, course_id, teacher_id, req.params.id]
    );
